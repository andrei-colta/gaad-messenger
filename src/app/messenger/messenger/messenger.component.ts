...
export class MessengerComponent implements OnInit, AfterViewChecked {
    ...
    constructor(private messengerService: MessengerService, private socketService: SocketService) { }
        ...
        setTimeout(() => {
            ...
            this.socketService.onMessage()
                .subscribe((message) => {
                    ...
                    console.log(message);
                    if (this.selectedFriend && (message.from.toString() === this.selectedFriend.toString() ||
    ...
    clearConversation() {
        this.messengerService.clearConversation(this.selectedFriend)
            .then((response) => {
                ...
                if (response.status === 'DataRetrieved') {
                    ...
                    this.myFriends.forEach(f => {
                        ...
                        this.socketService.unreadMessages.forEach(u => {
                            console.log(u, f);
                            if (u[0].toString() === f.id_user.toString() || u[0].toString() === f.id_friend.toString()) {
                                console.log('salut');
                                f.unread = u[1];
                            }
                        });
    ...
    getMessages() {
        this.messengerService.getMessages(this.selectedFriend)
            .then((response) => {
                console.log(response);
